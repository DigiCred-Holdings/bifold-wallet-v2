...
import { VDCard } from './components/VDCard'
import { TranscriptCard } from './components/TranscriptCard'
import { CredentialButtons } from './components'
...
interface CredentialCardProps {
  ...
  context: RenderContext
  onPress?: () => void
  showActions?: boolean // Add this prop to control action buttons
}
...
export class DefaultCredentialRenderer implements ICredentialRenderer {
  ...
  render(credential: CredentialExchangeRecord, context: RenderContext): React.ReactElement {
    ...
    return (
      <View>
        <VDCredentialCard credential={credential} context={context} onPress={handlePress} showActions={showActions}/>
        {showActions && <CredentialButtons isProcessing={false} onAccept={handleAccept} onDecline={handleDecline} />}
      </View>
    )
  }
}
...
export class VDCredentialRenderer implements ICredentialRenderer {
  ...
  render(credential: CredentialExchangeRecord, context: RenderContext): React.ReactElement {
    ...
    const showActions = credential.state === CredentialState.OfferReceived

    return (
      <View>
        <VDCredentialCard credential={credential} context={context} onPress={handlePress} showActions={showActions} />
        <CredentialButtons isProcessing={false} onAccept={handleAccept} onDecline={handleDecline} />
        {showActions && handleAccept && handleDecline && (
          <CredentialButtons isProcessing={false} onAccept={handleAccept} onDecline={handleDecline} />
        )}
      </View>
    )
